...
def _load_custom_model():
    """Load the local checkpoint or download it from Hugging Face."""
    model = models.resnet18(weights=None)
    model.fc = nn.Linear(model.fc.in_features, len(CUSTOM_CLASS_NAMES))
    model_path = CUSTOM_MODEL_PATH
    if not model_path.is_file():
        from huggingface_hub import hf_hub_download
        from settings import HF_MODEL_FILENAME, HF_MODEL_REPO
    ...
    # This project checkpoint is a legacy state-dict file trusted by the app.
